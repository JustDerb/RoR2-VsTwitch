    ...
    class LanguageOverride
    {
        ...
        public LanguageOverride()
        {
            ...
            StringsByToken = new Dictionary<string, string>
            {
                ...
                { "GENERIC_OUTRO_FLAVOR", $"..and so {STREAMER_TOKEN} left, knowing they shamelessly shook down their Twitch Chat for all their worth." },
                ...
                { "HUNTRESS_OUTRO_FLAVOR", $"..and so {STREAMER_TOKEN} left, knowing they would get absolutely CLAPPED if Twitch Chat had made them play literally any other character." },
                { "ENGI_OUTRO_FLAVOR", $"..and so {STREAMER_TOKEN} left, more BUNGUS than man, and yet, more man than Twitch Chat." },
                { "TREEBOT_OUTRO_FLAVOR", $"..and so {STREAMER_TOKEN} left, reduced to a lifeless desk plant after the Captain decided they needed their fuel array back." },
                { "CROCO_OUTRO_FLAVOR", $"..and so {STREAMER_TOKEN} left, reveling in the fact that, according to Twitch Chat, they are the GOODEST BOI." },
                { "LOADER_OUTRO_FLAVOR", $"..and so {STREAMER_TOKEN} left, knowing that they were the only viable melee character." },
                { "MERC_OUTRO_FLAVOR", $"..and so {STREAMER_TOKEN} left, their sword as dull as the idiots still watching this on Twitch." },
